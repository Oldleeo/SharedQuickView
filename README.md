...
只会识别以服务器地址开头的 UNC 路径，不会把普通文字、网址或本地磁盘路径当成共享目录打开。
...
开启后，软件监听剪贴板变化。复制到符合要求的 UNC 路径时，会立即替换 IP 并打开，无需再按 `Ctrl + V`。
...
2.0.2 已改为直接打开完整 UNC 目录。请确认使用的是最新版，并检查路径末尾是否在复制时被截断。
